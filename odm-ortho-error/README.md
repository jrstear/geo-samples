...
![Ortho dH vs distance from rasterizer origin — all four runs](images/aztec_validation_ortho_vs_distance.svg)

Both baselines overlap in a U-shape curve, which is eliminated with the PRs.
Reconstruction RMS_H stays constant across all four ODM runs (GCP≈0.020 ft,
CHK≈0.23 ft, see full reports).
...
Keeping the entire pipeline in topocentric CRS is an even better approach which should yield even better results - that architectural work is ongoing.
...
Full reports for all three ODM runs are available:
...
Each report contains per-point tables, per-point ortho crops with surveyed and tagged positions overlaid, the dH-vs-distance scatter, and a coloured overview map. All three were generated by `rmse.py` from the `geo` pipeline (see acknowledgements below).
